...
In the Lean development, the finite set `YY` is represented by an ordered list `ys`, and its subsets are represented by `ys.sublists`. The order inherited from the list is only used to compute the finite sum of shifts.
...
\prod_i \Delta_{y_i} \left(\frac{1}{D}\right)
  = \sum_{S \subseteq YY} \frac{(-1)^{|S|}}{D_S}.
$$
...
- The theorem is stated over a general commutative ring for arbitrary functions, and over a field for the inverse-function specialization.
- In Lean, division in a field is total, so the final theorem does not require separate nonzero-denominator assumptions.
